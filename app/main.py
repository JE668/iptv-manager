...
with engine.connect() as conn:
    try:
        with Session(engine) as session:
            if not session.get(Setting, "epg_days"): session.add(Setting(key="epg_days", value="3")); session.commit()
            # proxy_mode: 0-不代理, 1-仅udpxy, 2-全部代理
            if not session.get(Setting, "proxy_mode"): session.add(Setting(key="proxy_mode", value="1")); session.commit()
    except: pass
    ...
    async def subscribe(self, name: str, url: str, client_ip: str):
        ...
        if stream_id not in self.streams:
            self.streams[stream_id] = {
                ...
                "in_speed": "0 KB/s", "out_speed": "0 KB/s", "buffer_size": 100
            }
        ...
        try:
            while True:
                ...
                # 计算该流总计下游速度
                total_out = 0
                for client in self.streams[stream_id]["clients"].values():
                    try: total_out += float(client["speed"].split(' ')[0])
                    except: pass
...
async def merge_epg_data():
    ...
    if not sources:
        ...
        state.epg_gz = gzip.compress(state.epg_xml); return
...
async def startup_event(): asyncio.create_task(run_maintenance())
                    ...
                    if line.startswith("#EXTINF"):
                        ...
                        for j in range(i+1, min(i+5, len(lines))):
                            ...
                            if u.startswith("http") and u not in seen_urls:
                                # 智能代理判断
                                use_proxy = False
                                if proxy_mode == 2: use_proxy = True
                                elif proxy_mode == 1 and ("/udp/" in u or "/rtp/" in u): use_proxy = True
                                
                                final_url = f"{base_url}/live/{raw_name}?url={urllib.parse.quote(u, safe='')}" if use_proxy else u
                                unique_channels.append({"name": raw_name, "url": final_url})
                                seen_urls.add(u); break
            except: continue

    if request.url.path.endswith('.txt'):
        return Response(content="\n".join([f"{c['name']},{c['url']}" for c in unique_channels]), media_type="text/plain")
...
async def get_api_status(request: Request):
    ...
    return {
        "active_streams": active, "is_checking": state.is_checking,
        "kpis": {
            "total_in": f"{total_in:.1f} KB/s",
            "total_out": f"{total_out:.1f} KB/s",
...
# 其他路由(login, logout, add_source, delete_source等)同前...
...
async def del_e(eid: int):
    ...
    return RedirectResponse(url="/", status_code=303)
@app.get("/refresh")
async def ref(): asyncio.create_task(run_maintenance()); return RedirectResponse(url="/", status_code=303)
