...
with Session(engine) as session:
    try:
        ...
        if not session.get(Setting, "access_token"): session.add(Setting(key="access_token", value=""))
        session.commit()
    except: pass
                    ...
                    for line in lines:
                        ...
                        if line.startswith("#EXTINF"): cur_name = line.split(',')[-1].strip()
                            ...
                            if u not in seen_urls:
                                ...
                                unique_channels.append({"name": cur_name, "url": u, "use_proxy": use_p})
                                seen_urls.add(u)
                            cur_name = None
                else:
                    for line in lines:
                        ...
                        if ',' in line and not line.startswith("#"):
                            ...
                            if len(parts) == 2:
                                ...
                                if u and u not in seen_urls:
                                    use_p = force_proxy or (p_mode == 2) or (p_mode == 1 and ("/udp/" in u or "/rtp/" in u))
                                    unique_channels.append({"name": name, "url": u, "use_proxy": use_p})
                                    seen_urls.add(u)
            db_session.commit()
